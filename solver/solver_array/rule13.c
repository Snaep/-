	...
	for ( i = 2; i < 4; i++ )
	{
		...
		while ( Combinator_GetNext ( &c, combination ) == 0 )
		{
			...
			for ( j = 0; j <= i; j++ )
			{
				...
				for ( k = j + 1; k <= i; k++ )
				{
					AND ( sud->grid[combination[y]][j], sud->grid[combination[y]][k], temp, sud->length );
			...
			for ( j = 0; j < sud->length; j++ )
			{
				if ( cellok[j] != 0 )
				{
					AND ( sud->grid[y][j], subset, temp_sub, sud->length );
					OR ( changed, temp, changed, sud->length );

					INV ( subset, subset, sud->length );

					AND ( sud->grid[y][j], subset, sud->grid[y][j], sud->length );	//sud->grid[j][x] &= ( ~subset );
				}
			}
