...
int rule5 ( struct Sudoku* sud, unsigned int x, unsigned int y )
{
	...
	if ( POPCNT ( sud->grid[y][x], sud->length ) == 2 )
	{
		...
		{
			EXOR ( sud->grid[y][x], *sud->cellbox[y][x][i], temp, sud->length );
			if ( POPCNT ( temp, sud->length ) == 0 && ( &sud->grid[y][x] ) != sud->cellbox[y][x][i] )
			{
				changed = 0;
				for ( j = 0; j < sud->length; j++ )
				{
					AND ( *sud->cellbox[y][x][j], sud->grid[y][x], temp2, sud->length );
					OR ( temp3, temp2, temp3, sud->length );
